    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-[#1F2937] overflow-hidden shadow-sm sm:rounded-lg p-10 flex flex-col gap-y-5">
                ...
                <div class="item-card flex flex-col md:flex-row gap-y-10 justify-between md:items-center">
                    <div class="flex flex-row items-center gap-x-3">
                        ...
                        <div class="flex flex-col">
                            <h3 class="text-slate-200 text-xl font-bold">{{ $project->name }}</h3>
                            <p class="text-slate-500 text-sm">{{ $project->category->name }}</p>
                        </div>
                    </div>
                    <div class="hidden md:flex flex-col">
                        <p class="text-slate-500 text-sm">Budget</p>
                        <h3 class="text-slate-200 text-xl font-bold">Rp {{ number_format($project->budget, 0,
                            ',', '.') }}</h3>
                    </div>
                    <div class="hidden md:flex flex-col">
                        <p class="text-slate-500 text-sm">Applicants</p>
                        <h3 class="text-slate-200 text-xl font-bold">{{ $project->applicants->count() }}</h3>
                    </div>
                    <div class="hidden md:flex flex-col">
                        <p class="text-slate-500 text-sm">Client</p>
                        <h3 class="text-slate-200 text-xl font-bold">{{ $project->owner->name }}</h3>
                    </div>
                ...
                @empty
                <p class="text-slate-200">Belum ada data project terbaru</p>
                @endforelse
            </div>
        </div>
    </div>
